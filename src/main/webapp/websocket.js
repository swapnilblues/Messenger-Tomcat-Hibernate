...
async function translate(text){
    ...
    console.log(body)
   return await fetch('http://127.0.0.1:5000/translate', {
        method: 'POST',
...
async function connect() {
    ...
    const u = await fetch('http://localhost:8080/prattle/rest/user/auth', {
        method: 'POST',
    ...
    if(u.status!==500) {
        ...
        ws.onmessage = async function (event) {
            ...
            if(message.id==parentId && message.id!==0){
                ...
                // if (message.to === null) {
                //     log.innerHTML += message.from?message.from.username:message.username + " to all : " + message.content
                //     + " id: " + message.id + " " + message.attachment? "attachment"+message.attachment:"" + "\n";
                // } else {
                //     log.innerHTML +=
                //         message.from.username + " to " + message.to.username + " : " + message.content + " id: " + message.id + " " + message.attachment? " attachment "+message.attachment:"" + "\n";
                // }
            }
...
async function sendReplies(){
    to = document.getElementById("to").value;
    await fetch(`http://localhost:8080/prattle/rest/message/getExchangedMessages/user1/${username}/user2/${to}`)
...
async function replyToGroup(){
    var groupName = document.getElementById("toGroup").value;
    await fetch(`http://localhost:8080/prattle/rest/message/getExchangedMessages/group/${groupName}`)
        .then(mesresp=>mesresp.json())
        .then(messagesresp=>{
            messages = messagesresp;
            console.log(messagesresp)
